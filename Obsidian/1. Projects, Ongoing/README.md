...
### 필요 작업
- `스포일러 포함` / `보는 중` 필터링 추가
	- 헤더 부분에 구현한 `스포일러 포함 / 보는 중` 아이콘 자체에 추가할 수도 있고
	- 카테고리 필터에 추가할 수도 있겠다. 
	- 그런데 내 생각엔 `스포일러 포함 / 보는 중` 자체에 추가하는 게 더 맞는 것 같음. 필터 기능이긴 한데 카테고리 필터 기능은 아니니까.
- 목록에 나타나는 개별 카드 크기 줄이기?
	- 직접 데이터들을 넣어보면서 느끼는 건데, **포스터들이 다채로우니까 정신없다는 인상도 있다.** 하나의 정보가 차지하는 영역이 너무 크다는 인상도 있고?
	- 카드의 비율은 지금이 괜찮아 보임.
- 목록에서 불러오는 갯수
	- 최초 불러오기 : 이것도 두 줄씩 불러오면 충분할 듯. **지금은 딱 10개만 불러오는 문제가 있다.**
	- 스크롤 내릴 때 불러오는 갯수 : 두 줄씩 불러오면 충분해 보임
	- 위의 카드 크기를 줄였을 때 한 줄에 몇 개씩 들어가는가를 반영해서 최초에도 두 줄, 스크롤을 내렸을 때도 두 줄씩 불러오는 방식으로 수정하자.
	- 지금 예를 들면 한 줄에 3/4/6개가 들어가는데, 카드 크기를 줄이고 갯수를 늘리더라도 공배수에 맞출 수 있게끔 만들면 될 듯.

- 당장 할 필요 없음) 검색어 추가
	- 예를 들면 `흘러가는 나날, 밥은 맛있어`는 `히비메시`라고 불리기도 한단 말임? 이거를 글 작성할 때 함께 적어넣는 것이다.
