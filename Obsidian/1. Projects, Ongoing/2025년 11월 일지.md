...
>- 발생 중인 이슈
>	1. 메뉴가 제대로 나타나지 않는 현상
>	2. 원래 캔버스로 되돌리는 과정에서 레이아웃이 깨지는 현상
>		- 이거는 `TutorialCanvas`에 아예 해당 버튼을 복붙하는 식으로 구현할 수 있지 않을까 싶은데, 내일 생각해보기로 함.

>[!question]
>- 오브젝트 찾기 : `FindObjectByType`을 쓰는 대신 더 좋은 방법이 있을까?
