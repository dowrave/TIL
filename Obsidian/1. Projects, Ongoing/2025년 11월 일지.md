...
>[!wip]
>- 하이라이트 구현 후 잘 동작하는지 점검 중
>- 발생 중인 이슈
>	- 튜토리얼
>		- `OperatorSlot(op_sniper)` 하이라이트 과정에서 튜토리얼 캔버스로 버튼이 옮겨질 때도 있고 옮겨지지 않을 때도 있는 현상
>	- `OperatorInventoryPanel`의 왼쪽 부분을 `tutorialCanvas`로 옮길 때, 스킬 버튼이 제대로 옮겨지지 않는 현상
>		- 신기한 건 스탯은 그대로 옮겨지는데 스킬 버튼 이미지가 제대로 초기화되지 않음
>		- 뒤로 가기 / 홈 버튼 UI가 옮겨진 UI 요소에 의해 가려지는 현상이 있음
>			- (생각) 이거는 `tutorialStep`에서 하이라이트 되는 요소를 여러 개로 설정하면 되지 않을까 싶음
>	- `StageScene`의 `DeployableBox`가 새로운 캔버스에 나타났을 때 `Update`가 동작하는 현상
>		- (구현)`Initialize`된 경우에만 `Update`가 동작하게끔 구현해봄
>	- `OperatorSlot(op_sniper)` : 하이라이트 과정에서 튜토리얼 캔버스로 버튼이 옮겨질 때도 있고 옮겨지지 않을 때도 있는 현상
>	- 씬 전환 시 이전 `tutorialCanvas`가 남아 있는 현상
# 251119 
