...
> 곧 할 일
> - 버프 이펙트 VFX로 구현하기 : 기존엔 파티클들로만 했는데, `Trail` 같은 것도 VFX로 지원하므로 정보를 찾아보면 뭐가 더 나올 듯? 이런 건 AI한테 물어봤을 때 대답이 나오는 소재는 아니다.
> - 팔레트 스왑을 해도 좋으니 이펙트들을 하나씩이라도 구현해보고 여기저기 적용해보자
> 	- 근거리 오퍼레이터의 공격 이펙트
> 		- 자세히 보니 스프라이트에 이펙트를 연동하는 식으로 스파인에서는 구현하던데, 지금 프로젝트에서는 일단은 스프라이트까지 작업하지는 못할 것 같음
> 	- 버프 이펙트

- 이후에 스킬 여러 종류 구현하고 스테이지들도 구현하고 하면 될 듯
