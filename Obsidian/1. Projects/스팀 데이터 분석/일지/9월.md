...
- `230915~` : 텐서플로우 2.X 버전에서 수행하기로 함. 1버전에서 `estimator`를 이용한 학습에서, 학습이 진행되지 않는 현상이 있었다.
...
- 그래서 그간 텐서플로우 1.x 버전의 익숙하지 않은 `estimator`를 사용하는 방법 대신, 텐서플로우 2.x 버전의 케라스 API를 이용할 것이다.
